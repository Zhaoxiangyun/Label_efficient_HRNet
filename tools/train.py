...
from core.criterion import CrossEntropy, OhemCrossEntropy, ContrastiveLoss
...
def main():
    ...
    if config.MODEL.CONTRASTIVE:
       criterion = ContrastiveLoss(ignore_label=config.TRAIN.IGNORE_LABEL)
    model = FullModel(model, criterion, config.MODEL.CONTRASTIVE)
    #if not config.MODEL.CONTRASTIVE:
    if distributed:
        ...
        if args.local_rank == 0:
            ...
            if (epoch+1)%30 == 0:    
             torch.save(model.state_dict(),
                       os.path.join(final_output_dir, 'epoch_'+str(epoch) + '_check_state.pth'))            
             logger.info('=> saving checkpoint to {}'.format(
                final_output_dir + '/epoch_'+str(epoch)+'_check_state.pth'))
